...
epsoc3 <- read_spss(paste0(path,"181122 - EPSOC Base parcial 5.sav"))

names(epsoc3) <- tolower(gsub("_", ".", names(epsoc3)))
```
...
labs.epsoc3 <- makeVlist(epsoc3)
labs.epsoc3 <- labs.epsoc3 %>% 
                        mutate(label2 = map_chr(label, toString))
```

# General

La actual base cuenta con `r dim(epsoc3)[1]` casos recogidos entre el `r min(as.Date(epsoc3$date))` y el `r max(as.Date(epsoc3$date))`. 
...
epsoc3$dura.d <- ifelse(nchar(epsoc3$duration) == 8,
                 0, 
                 as.numeric(gsub("\\..*$", "", epsoc3$duration)))

epsoc3$duracion <- hms(gsub("^.*\\.", "", epsoc3$duration))
epsoc3$duracion.t <- as.duration(epsoc3$duracion + days(epsoc3$dura.d))
epsoc3$duracion.t.min <- epsoc3$duracion.t@.Data/60

epsoc3 %>% 
        ggplot(aes(x = duracion.t.min)) + 
        geom_histogram(binwidth = 5) +
        theme_bw() +
        ggtitle("Distribución duración entrevistas (escala truncada < 150 minutos)") +
...
Existen `r length(epsoc3$folio[epsoc3$duracion.t.min < 20])` entrevistas que duran menos de 20 minutos, estas debieran ser supervisadas.


```{r}
knitr::kable(cbind(epsoc3$folio[epsoc3$duracion.t.min < 20],
                (round(epsoc3$duracion.t.min[epsoc3$duracion.t.min < 20],1))), 
             col.names = c("Folio", "Duración (minutos)"),
...
Existen `r length(epsoc3$folio[epsoc3$duracion.t.min > 150])` entrevistas que duran más de 150 minutos, estas debieran ser supervisadas.

```{r ent.largas}
knitr::kable(cbind(epsoc3$folio[epsoc3$duracion.t.min > 150],
                (round(epsoc3$duracion.t.min[epsoc3$duracion.t.min > 150]/60,1))), 
             col.names = c("Folio", "Duración (horas)"),
...
`r ifelse(sum(is.na(epsoc3$latitude)) == 0, "No existen entrevistas sin datos de georreferenciación", paste("Existen", sum(is.na(epsoc3$latitude))))` entrevistas sin datos de georreferenciación:
...
kable(epsoc3 %>% 
        ...
        arrange(srvyr),
      digits = 1, 
      col.names = c("Folio", "Encuestador", "Total encuestas", "% sin georef.")) %>% 
        kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                      full_width=F)
```
...
## Comienzo viñetas
epsoc3$time2 <- ifelse(nchar(epsoc3$time2) == 30, 
                      as.character(strptime(epsoc3$time2,
                              format = "%a %b %d %H:%M:%S %z %Y")), 
                      as.character(ymd_hms(epsoc3$time2)-hours(3)))
epsoc3$time2 <- as_datetime(epsoc3$time2)
epsoc3$time2.hms <- format(epsoc3$time2, format = "%H:%M:%S")
epsoc3$time2.hms <- as.POSIXct(epsoc3$time2.hms, format = "%H:%M:%S")
epsoc3$time2.dmy <- dmy(format(epsoc3$time2, format = "%d-%m-%y"))

## Fin viñetas
epsoc3$time3 <- ifelse(nchar(epsoc3$time3) == 30, 
                      as.character(strptime(epsoc3$time3,
                                            format = "%a %b %d %H:%M:%S %z %Y")), 
                      as.character(ymd_hms(epsoc3$time3)-hours(3)))
epsoc3$time3 <- as_datetime(epsoc3$time3)
epsoc3$time3.hms <- format(epsoc3$time3, format = "%H:%M:%S")
epsoc3$time3.hms <- as.POSIXct(epsoc3$time3.hms, format = "%H:%M:%S")
epsoc3$time3.dmy <- dmy(format(epsoc3$time3, format = "%d-%m-%y"))

epsoc3$dura.vinetas <- difftime(epsoc3$time3, epsoc3$time2,
                               units = "mins")
ggplot(epsoc3, aes(x = time2.dmy, y = time2.hms)) + geom_point() +
        labs(x = "Día", y = "Hora") + 
        ggtitle("Día y hora comienzo actividad viñetas") + 
        theme_bw()

ggplot(epsoc3, aes(x = srvyr, y = time2.hms)) + 
  geom_point() +
...
```{r plot.dura.enc}
ggplot(epsoc3, aes(x = srvyr, y = dura.vinetas)) + 
  geom_point() +
...
```{r}
item.just <- flatten_chr(labs.epsoc3[grep("usto", labs.epsoc3$label2),1])
...
```{r}
epsoc3$duration <- chron(times=epsoc3$duration)
cf <- check_that(epsoc3, edad.seleccionado <= 59 & edad.seleccionado >= 18,
...
epsoc3$enc.edad[as.character(epsoc3$enc.edad) == "1582-10-14"] <- NA # comportamiento extraño al importar desde SPSS
edad <- tibble(Folio = epsoc3$folio[is.na(epsoc3$enc.edad)],
        Fecha = epsoc3$enc.edad[is.na(epsoc3$enc.edad)],
        Edad = epsoc3$edad.seleccionado[is.na(epsoc3$enc.edad)]) 
knitr::kable(edad,
...
```{r sexo}
knitr::kable(cbind(epsoc3$folio[epsoc3$sexo.enc != epsoc3$sexo.seleccionado],
                epsoc3$sexo.enc[epsoc3$sexo.enc != epsoc3$sexo.seleccionado],
                epsoc3$sexo.seleccionado[epsoc3$sexo.enc != epsoc3$sexo.seleccionado]), 
